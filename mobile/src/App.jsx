...
// 依恋类型小测：3 题测焦虑维度、3 题测回避维度
const ATTACHMENT_Q = [
  { key: 'anx1', dim: 'anx', text: '我常担心对方其实没那么喜欢我，或可能会离开' },
  { key: 'anx2', dim: 'anx', text: '我需要对方常常确认对我的心意，否则会不安' },
  { key: 'anx3', dim: 'anx', text: '对方没及时回应时，我容易胡思乱想、情绪起伏' },
  { key: 'avo1', dim: 'avo', text: '我不太习惯向另一半倾诉内心，或依赖对方' },
  { key: 'avo2', dim: 'avo', text: '关系太亲密时我会感到不自在，想要自己的空间' },
  { key: 'avo3', dim: 'avo', text: '比起黏在一起，我更看重独立和个人空间' },
];
const ATTACHMENT_SCALE = [['很不符合', 1], ['有点', 2], ['比较符合', 3], ['很符合', 4]];
function computeAttachment(ans) {
  const anx = ['anx1', 'anx2', 'anx3'].map(k => ans[k]).filter(Boolean);
  const avo = ['avo1', 'avo2', 'avo3'].map(k => ans[k]).filter(Boolean);
  if (anx.length < 3 || avo.length < 3) return null; // 未答完不判定
  const a = anx.reduce((s, x) => s + x, 0) / 3;
  const v = avo.reduce((s, x) => s + x, 0) / 3;
  const hiA = a >= 2.5, hiV = v >= 2.5;
  const type = hiA && hiV ? '混乱型' : hiA ? '焦虑型' : hiV ? '回避型' : '安全型';
  return { type, anxiety: Math.round(a * 10) / 10, avoidance: Math.round(v * 10) / 10 };
}
...
const BLANK = {
  ...
  attachment_answers:'', attachment_style:'', attachment_scores:'',
};
...
// 依恋类型小测
function AttachmentQuiz({ form, set }) {
  let ans = {};
  try { ans = form.attachment_answers ? JSON.parse(form.attachment_answers) : {}; } catch { ans = {}; }
  function pick(key, val) {
    const next = { ...ans, [key]: val };
    set('attachment_answers', JSON.stringify(next));
    const r = computeAttachment(next);
    set('attachment_style', r ? r.type : '');
    set('attachment_scores', r ? JSON.stringify({ anxiety: r.anxiety, avoidance: r.avoidance }) : '');
  }
  return (
    <div className="space-y-4">
      {ATTACHMENT_Q.map(q => (
        <div key={q.key}>
          ...
          <div className="flex gap-1.5">
            {ATTACHMENT_SCALE.map(([label, val]) => (
              <button key={val} type="button" onClick={() => pick(q.key, val)}
                className={`flex-1 py-2 rounded-lg text-xs border transition-all ${ans[q.key] === val ? 'border-primary-400 bg-primary-50 text-primary-700 font-medium' : 'border-gray-200 text-gray-500 bg-white'}`}>
                {label}
              </button>
            ))}
          </div>
      ...
      {form.attachment_style && (
        <div className="bg-primary-50 rounded-xl px-4 py-3 text-sm text-primary-700">
          你的依恋倾向：<strong>{form.attachment_style}</strong>
          <span className="text-xs text-primary-400 ml-1">仅用于更精准的关系匹配</span>
        </div>
      )}
    </div>
  );
...
function StepPersonality({ form, set }) {
  return (
    <>
      ...
      <Field label="依恋类型小测（选填）" hint="凭第一直觉作答，用于更精准的关系匹配">
        <AttachmentQuiz form={form} set={set} />
      </Field>
